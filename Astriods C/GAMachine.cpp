...
void GAMachine::Update(float dt)
{
    ...
    sprintf_s(buffer,"G: %d    Best Fitness %3.2f",m_generation,m_bestFitness);
                                  ...
                                  vector<Gene>&offspring1,vector<Gene>&offspring2)
{
    ...
    {
        for(unsigned int j=0;j<usedGenes.size();++j)
        {
    ...
    usedGenes.clear();
    genePosition = 0;
    for (unsigned int i=0;i<usedGenesPositions.size();++i)
...
void GAMachine::MutateOffset(vector<Gene> &genes)
{
    ...
    for (unsigned int gene=0; gene<genes.size(); ++gene)
    {
